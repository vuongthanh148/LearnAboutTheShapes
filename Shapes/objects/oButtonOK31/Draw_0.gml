...
if(chosenRight == -1){  // chua chon dap an
	if(global.enableAns) draw_roundrect_colour(742, 400, 804, 440, $e5c66e, $e5c66e , false);
	if(global.enableAns) draw_sprite(sAnswerImage,in,774,290);
	global.data[global.typesQuestion[global.questionIndex]-1] = false;
}
else if(chosenRight == 0){ //chon dap an sai
	if(global.enableAns){
		draw_roundrect_colour(742, 400, 804, 440, $6857ff, $6857ff , false);
		draw_sprite(sAnswerImage,in,774,290);
	}
	timer += 1;
	if(timer >= 90){
		timer = 0;
	...
	if(global.enableAns) draw_roundrect_colour(742, 400, 804, 440, $82bf8f, $82bf8f , false);
	timer += 1;

	if(timer >= room_speed * 1.5){ //START MOVING
		global.enableAns = false;
		posX += answerSpeed;
		if(posX >= 800 and answerSpeed > 0) {
			answerSpeed = -12;
		}
	...
	if(timer >= 1.75 * room_speed) {
		...
		draw_text((posX + 774)/2 - string_width("looks like")/2, 290 - string_height("looks like")/2, "looks like");
	}
	if(timer >= room_speed * 5){
		global.nextAns = true;
	}
	draw_sprite(sImageShasow,in,774,290);
